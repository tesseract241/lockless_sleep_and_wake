#include <atomic>
void sleep(std::atomic_int32_t &flag, int32_t expected);
...
void sleep(std::atomic_int32_t &flag, int32_t expected){
    while(expected==flag.load()){
        syscall(SYS_futex, &flag, FUTEX_WAIT_PRIVATE, &expected, NULL, NULL, NULL);
    }
}
...
void sleep(std::atomic_int32_t &flag, int32_t expected){
    while(expected==flag.load()){
        WaitOnAddress(&flag, &expected, 4, INFINITE);
    }
}
void wake_one(std::atomic_int32_t &flag, int32_t desired){
    flag.store(desired);
